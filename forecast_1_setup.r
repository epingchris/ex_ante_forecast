# This script generates a dataframe containing the information needed for subsequent analysis
#
# It requires the following inputs:
# 1. project_dir: absolute path of the directory containing TMF implementation output
# 2. polygon_dir: absolute path of the directory containing project shapefiles
# 3. out_path: absolute path of the directory where output files will be saved
...
#
# 1. out_path/out_project_var.csv: dataframe containing project-level variables, with columns:
## project: project ID
...
## prj_elev: mean elevation of project pixels
## prj_slope: mean slope of project pixels
## prj_remote: mean remoteness of project pixels
## reg_elev: mean elevation of regional pixels
## reg_slope: mean slope of regional pixels
## reg_remote: mean remoteness of regional pixels
## gdppc_mean: mean GDP per capita of project country five years prior to project start
## gdppc_rate: annual compound growth rate of GDP per capita of project country five years prior to project start
## wgicc_mean: mean World Governance Index Control of Corruption indicator of project country five years

#2. out_path/out_project_status.csv: dataframe containing project status check results, with columns:
## project: project ID
## carbon_complete: whether carbon density values for LUC 1, 2, 3

#3. out_path/out_table_s2.csv: table of reference carbon density values per land class for each project, with standard errors and number of GEDI beams used
## project: project ID
## cdens_1 to cdens_6: reference carbon density (MgC ha-1) per land class
## n_1 to n_6: number of GEDI beams used to calculate reference carbon density per land class
## se_1 to se_6: standard error of reference carbon density per land class
...
out_path = "/maps/epr26/ex_ante_forecast_out/" #path to store script output
...
projects_all = FindFiles(project_dir, exclude = exclude_strings) %>%
  as.numeric() %>%
...
#Output
write.csv(projects_status, paste0(out_path, "out_project_status.csv"), row.names = F) #project status check results
write.csv(project_var, paste0(out_path, "out_project_var.csv"), row.names = F) #project-level variables
...
write.csv(project_cdens_var, paste0(out_path, "out_table_s2.csv"), row.names = F)

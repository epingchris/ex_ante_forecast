# This script reads the k.parquet and matches.parquet files of each project generated by the TMF implementation code,
# retrieves and saves as csv files the following three time series for 100 subsampled pixel sets of each project:
# 1. Ex post additionality (out_additionality_[proj].csv)
# 2. Historical project carbon loss rate (out_closs_project_[proj].csv)
# 3. Historical regional carbon loss rates (out_closs_regional_[proj].csv)
# 4. Observed counterfactual carbon loss rates (out_closs_obs_cf_[proj].csv)
# 5. Observed project carbon loss rates (out_closs_obs_p_[proj].csv)
# It then calculates and saves as csv files the bootstrapped mean over the 100 sets of each project:
# 6. Ex post additionality (boot_additionality_[proj].csv)
# 7. Historical project carbon loss rate (boot_closs_project_[proj].csv)
# 8. Historical regional carbon loss rates (boot_closs_regional_[proj].csv)
# 9. Observed counterfactual carbon loss rates (boot_closs_obs_cf_[proj].csv)
# 10. Observed project carbon loss rates (boot_closs_obs_p_[proj].csv)
#
# It requires the following inputs:
# 1. project_dir: absolute path of the directory containing TMF implementation output
# 2. out_path: absolute path of the directory where output files will be saved
# 3. out_path/out_project_var.csv: dataframe containing project-level variables
# 4. project_dir/<proj>/pairs/k.parquet: TMF implementation data for project pixels
# 5. project_dir/<proj>/pairs/matches.parquet: TMF implementation data for loosely matched pixels
#
# The additionality time series data (1) contain the following columns:
## year: year
## pair: subsample ID
...
#
# The project/regional carbon loss time series data (2-5) contain the following columns:
## year: year
## pair: subsample ID
## counterfactual: carbon density in the counterfactual scenario at the given year (MgC ha-1)
## project: carbon density in the project area at the given year (MgC ha-1)
...
#
# The bootstapped mean time series data (6-10) contain the following columns:
## year: year
## closs_annual: bootstrapped mean annual compound carbon loss rate from the given year to t0 (MgC ha-1 yr-1)
## type: "project", "counterfactual", or "regional"
...
out_path = "/maps/epr26/ex_ante_forecast_out/" #path of files generated by this repo, including in forecast_1_setup.r

project_var = read.csv(paste0(out_path, "out_project_var.csv"), header = T)
...
for(i in seq_along(projects)) {
  ...
  write.csv(additionality, paste0(out_path, "out_additionality_", projects[i], ".csv"), row.names = F)
  write.csv(obs_counterfactual, paste0(out_path, "out_closs_obs_cf_", projects[i], ".csv"), row.names = F)
  write.csv(obs_project, paste0(out_path, "out_closs_obs_p_", projects[i], ".csv"), row.names = F)
  write.csv(historical_project, paste0(out_path, "out_closs_project_", projects[i], ".csv"), row.names = F)
  ...
  write.csv(historical_region, paste0(out_path, "out_closs_regional_", projects[i], ".csv"), row.names = F)
}
...
for(i in seq_along(projects)) {
  ...
  additionality = read.csv(paste0(out_path, "out_additionality_", projects[i], ".csv"), header = T)
  closs_obs_cf = read.csv(paste0(out_path, "out_closs_obs_cf_", projects[i], ".csv"), header = T)
  closs_obs_p = read.csv(paste0(out_path, "out_closs_obs_p_", projects[i], ".csv"), header = T)
  closs_project = read.csv(paste0(out_path, "out_closs_project_", projects[i], ".csv"), header = T)
  closs_regional = read.csv(paste0(out_path, "out_closs_regional_", projects[i], ".csv"), header = T)
...
write.csv(boot_additionality_arith_df, paste0(out_path, "out_boot_additionality_arith.csv"), row.names = F)
write.csv(boot_additionality_geom_df, paste0(out_path, "out_boot_additionality_geom.csv"), row.names = F)
write.csv(boot_closs_obs_cf_df, paste0(out_path, "out_boot_closs_obs_cf.csv"), row.names = F)
write.csv(boot_closs_obs_p_df, paste0(out_path, "out_boot_closs_obs_p.csv"), row.names = F)
write.csv(boot_closs_project_df, paste0(out_path, "out_boot_closs_project.csv"), row.names = F)
write.csv(boot_closs_region_df, paste0(out_path, "out_boot_closs_regional.csv"), row.names = F)
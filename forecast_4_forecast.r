# This script generates project-based, region-based, and mixed forecasts of carbon loss rates.
# It requires the following inputs:
# 1. boot_closs_obs_cf.csv: observed counterfactual carbon loss rates
# 2. boot_closs_obs_p.csv: observed project carbon loss rates
# 3. boot_closs_project.csv: historical project carbon loss rates
# 4. boot_closs_regional.csv: historical regional carbon loss rates
#
# The output includes:
# 1. Forecasts of carbon loss rates for each project over different target periods.
# 2. Visualizations comparing the performance of different forecasting methods.
...
in_path = "../03_Project_forecasting/supporting_data/" #path of files generated by this repo, including in forecast_1_setup.r
out_path = "../03_Project_forecasting/" #path of files generated by this repo, including in forecast_1_setup.r
fig_path = "../03_Project_forecasting/manuscript/" #path of output figures
...
boot_closs_obs_cf = read.csv(paste0(in_path, "boot_closs_obs_cf.csv"), header = T)
boot_closs_obs_p = read.csv(paste0(in_path, "boot_closs_obs_p.csv"), header = T)
boot_closs_project = read.csv(paste0(in_path, "boot_closs_project.csv"), header = T)
boot_closs_region = read.csv(paste0(in_path, "boot_closs_regional.csv"), header = T)
...
forecast_all = bind_rows(forecast_prj, forecast_reg, forecast_mix) %>%
  ...
  rename(closs_obs_cf = rate_obs.x, closs_obs_p = rate_obs.y)

write.csv(forecast_all, paste0(out_path, "out_forecast.csv"), row.names = F)
forecast_all = read.csv(paste0(out_path, "out_forecast.csv"), header = T)
...
write.csv(forecast_summ, paste0(out_path, "out_forecast_summary.csv"), row.names = F)
forecast_summ = read.csv(paste0(out_path, "out_forecast_summary.csv"), header = T)
...
ggsave(paste0(fig_path, "figure_6_overall_forecast_performance.png"), width = 40, height = 60, unit = "cm", dpi = 300)


# Determine the best forecasts ----

#Calculate rankings of r2, MAE and RMSE for the 5-year target period
forecast_rank = forecast_summ %>%
...
best_rank = forecast_rank %>%
  filter(sum_rank <= quantile(sum_rank, 0.1, na.rm = T)) %>%
  dplyr::select(type, period_hist_prj, period_hist_reg, gof, mae, rmse, sum_rank) %>%
  arrange(sum_rank)

write.csv(best_rank, paste0(fig_path, "table_s3_best_rank.csv"), row.names = F)
...
forecast_obs_var = forecast_all %>%
  filter(period_hist_prj == -8 & period_hist_reg == -2 & period_target == 5) %>%
  dplyr::select(!c(period_hist_prj, period_hist_reg, type)) %>%
  pivot_wider(names_from = period_target, values_from = c(forecast, closs_obs_cf, closs_obs_p))

model_df = list(envir_var, c_init_var, forecast_obs_5_var) %>% reduce(left_join, by = "project") %>%
  mutate(credit = c_init * ((1 - closs_obs_p) ^ 5) - c_init * ((1 - closs_obs_cf) ^ 5))
...
var_to_scale = c("area_ha", "prj_slope", "prj_remote", "gdppc_mean", "gdppc_rate", "wgicc_mean", "forecast")
model_df_scaled = model_df_trimmed %>%
  mutate(across(all_of(var_to_scale), ~ as.numeric(scale(.x))))
write.csv(model_df_scaled, paste0(out_path, "out_model_df_scaled.csv"), row.names = F)
model_df_scaled = read.csv(paste0(out_path, "out_model_df_scaled.csv"), header = T) %>%
  rename_with(~ str_replace(.x, "_5$", ""), ends_with("_5")) %>%
  dplyr::select(!ends_with("_10"))

# Predictive models ----

#Counterfactual carbon loss: naive model (only forecast is used)
plot_cf = PlotModel(input = model_df_trimmed, obs_var = "closs_obs_cf", model = "naive")

#Project carbon loss: model selection
plot_p = PlotModel(input = model_df_trimmed, obs_var = "closs_obs_p", model = "sel")

#Difference in carbon loss: model selection
plot_reduc = PlotModel(input = model_df_trimmed, obs_var = "add_rate", model = "forecast")

#Carbon credit production (emissions reductions): model selection
plot_credit = PlotModel(input = model_df_trimmed, obs_var = "credit", model = "sel")
...
plot_p_uni = PlotModel(input = model_df_trimmed, obs_var = "closs_obs_p", model = retained_var)
plot_credit_uni = PlotModel(input = model_df_trimmed, obs_var = "credit", model = retained_var)
...
plot_fig6abc = plot_cf$plot / plot_p$plot / plot_reduc$plot +
  plot_layout(guide = "collect", axes = "collect", axis_titles = "collect", heights = c(1, 1, 1))
ggsave(paste0(fig_path, "figure_7_obs_pred_abc.png"), plot = plot_fig6abc, width = 60, height = 60, unit = "cm", dpi = 300)

plot_fig6d = plot_credit$plot
ggsave(paste0(fig_path, "figure_7_obs_pred_d.png"), plot = plot_fig6d, width = 60, height = 60, unit = "cm", dpi = 300)